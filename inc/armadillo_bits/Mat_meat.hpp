...
template <typename eT>
inline Mat<eT>::Mat()
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
}
...
template <typename eT>
inline Mat<eT>::Mat(const uword in_n_rows, const uword in_n_cols)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const SizeMat &s)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem(s.n_rows * s.n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const uword in_n_rows, const uword in_n_cols, const fill::fill_class<fill_type> &f)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename fill_type>
inline Mat<eT>::Mat(const SizeMat &s, const fill::fill_class<fill_type> &f)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem(s.n_rows * s.n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const arma_vec_indicator &, const uhword in_vec_state)
    : n_rows((in_vec_state == 2) ? 1 : 0), n_cols((in_vec_state == 1) ? 1 : 0), n_elem(0), vec_state(in_vec_state), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
}
...
//! constructor used by Row and Col classes
template <typename eT>
inline Mat<eT>::Mat(const arma_vec_indicator &, const uword in_n_rows, const uword in_n_cols, const uhword in_vec_state)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols), vec_state(in_vec_state), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const arma_fixed_indicator &, const uword in_n_rows, const uword in_n_cols, const uhword in_vec_state, const eT *in_mem)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem(in_n_rows * in_n_cols), vec_state(in_vec_state), mem_state(3), mem(in_mem)
{
  arma_extra_debug_sigprint_this(this);
}
...
Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols)
{
  ...
  if (t_vec_state > 0)
  {
    ...
    else
    {
      if (t_vec_state == 1)
      {
        arma_debug_set_error(err_state, err_msg, (in_n_cols != 1), "Mat::init(): requested size is not compatible with column vector layout");
      }
      if (t_vec_state == 2)
      {
        arma_debug_set_error(err_state, err_msg, (in_n_rows != 1), "Mat::init(): requested size is not compatible with row vector layout");
      }
    }
  }
...
template <typename eT>
inline Mat<eT>::Mat(const char *text)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
//! create the matrix from a textual description
template <typename eT>
inline Mat<eT>::Mat(const std::string &text)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const std::vector<eT> &x)
    : n_rows(uword(x.size())), n_cols(1), n_elem(uword(x.size())), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const std::initializer_list<eT> &list)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(Mat<eT> &&X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
...
template <typename eT>
inline Mat<eT>::Mat(const Mat<eT> &in_mat)
    : n_rows(in_mat.n_rows), n_cols(in_mat.n_cols), n_elem(in_mat.n_elem), vec_state(0), mem_state(0), mem()
{
...
Mat<eT>::swap(Mat<eT> &B)
{
  ...
  if ((A_mem_state == 0) && (B_mem_state == 0) && layout_ok)
  {
    ...
    else if ((A_use_local_mem == true) && (B_use_local_mem == true))
    {
      ...
      for (uword ii = 0; ii < N; ++ii)
      {
        std::swap(A_mem_local[ii], B_mem_local[ii]);
      }
    }
...
template <typename eT>
inline Mat<eT>::Mat(eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols, const bool copy_aux_mem, const bool strict)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem(aux_n_rows * aux_n_cols), vec_state(0), mem_state(copy_aux_mem ? 0 : (strict ? 2 : 1)), mem(copy_aux_mem ? 0 : aux_mem)
{
  arma_extra_debug_sigprint_this(this);
...
//! the array is copied.
template <typename eT>
inline Mat<eT>::Mat(const eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem(aux_n_rows * aux_n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const char junk, const eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem(aux_n_rows * aux_n_cols), vec_state(0), mem_state(3), mem(aux_mem)
{
  arma_extra_debug_sigprint_this(this);
  arma_ignore(junk);
}
...
inline Mat<eT>::Mat(const BaseCube<eT, T1> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const subview<eT> &X, const bool use_colmem)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(use_colmem ? 3 : 0), mem(use_colmem ? X.colptr(0) : NULL)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const subview<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const subview_row_strans<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const subview_row_htrans<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const xvec_htrans<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const xtrans_mat<eT, do_conj> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const subview_cube<eT> &x)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const diagview<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Mat<eT>::Mat(const spdiagview<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(X.n_elem), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1>
inline Mat<eT>::Mat(const subview_elem1<eT, T1> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename T2>
inline Mat<eT>::Mat(const subview_elem2<eT, T1, T2> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1>
inline Mat<eT>::Mat(const SpBase<eT, T1> &m)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
//! creation of subview (row vector)
template <typename eT>
arma_inline const subview_row<eT>
...
//! creation of subview (column vector)
template <typename eT>
arma_inline const subview_col<eT>
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem1<eT, T1>
...
template <typename eT>
template <typename T1, typename T2>
arma_inline const subview_elem2<eT, T1, T2>
...
template <typename eT>
template <typename T1>
arma_inline const subview_elem2<eT, T1, T1>
...
template <typename eT>
template <typename T2>
arma_inline const subview_elem2<eT, T2, T2>
...
template <typename eT>
arma_inline const subview_each1<Mat<eT>, 0>
...
template <typename eT>
arma_inline const subview_each1<Mat<eT>, 1>
...
template <typename eT>
template <typename T1>
inline subview_each2<Mat<eT>, 0, T1>
...
template <typename eT>
template <typename T1>
inline subview_each2<Mat<eT>, 1, T1>
...
template <typename eT>
template <typename T1>
inline const subview_each2<Mat<eT>, 0, T1>
...
template <typename eT>
template <typename T1>
inline const subview_each2<Mat<eT>, 1, T1>
...
inline Mat<eT>::Mat(const Gen<T1, gen_type> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem(n_rows * n_cols), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const Op<T1, op_type> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const eOp<T1, eop_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()), n_elem(X.get_n_elem()), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename op_type>
inline Mat<eT>::Mat(const mtOp<eT, T1, op_type> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const Glue<T1, T2, glue_type> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
inline Mat<eT>::Mat(const eGlue<T1, T2, eglue_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()), n_elem(X.get_n_elem()), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Mat<eT>::Mat(const mtGlue<eT, T1, T2, glue_type> &X)
    : n_rows(0), n_cols(0), n_elem(0), vec_state(0), mem_state(0), mem()
{
  arma_extra_debug_sigprint_this(this);
...
Mat<eT>::fill(const fill::fill_class<fill_type> &)
{
  ...
  if (is_same_type<fill_type, fill::fill_zeros>::yes)
    (*this).zeros();
  if (is_same_type<fill_type, fill::fill_ones>::yes)
    (*this).ones();
...
Mat<eT>::save(const std::string name, const file_type type, const bool print_status) const
{
  ...
  {
    arma_debug_warn("Mat::save(): couldn't write to ", name);
  }
...
Mat<eT>::save(std::ostream &os, const file_type type, const bool print_status) const
{
  ...
  if (print_status && (save_okay == false))
  {
    arma_debug_warn("Mat::save(): couldn't write to the given stream");
  }
...
Mat<eT>::load(const std::string name, const file_type type, const bool print_status)
{
  ...
  case hdf5_binary_trans:
  {
    ...
    if (load_okay)
    {
      op_strans::apply_mat_noalias(*this, tmp);
    }
  }
  break;
...
template <typename eT>
inline Mat<eT>::row_iterator::row_iterator(Mat<eT> &in_M, const uword in_row)
    : M(in_M), row(in_row), col(0)
{
  arma_extra_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator(const Mat<eT> &in_M, const uword in_row)
    : M(in_M), row(in_row), col(0)
{
  arma_extra_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::const_row_iterator::const_row_iterator(const typename Mat<eT>::row_iterator &X)
    : M(X.M), row(X.row), col(X.col)
{
  arma_extra_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::row_col_iterator::row_col_iterator()
...
template <typename eT>
inline Mat<eT>::row_col_iterator::row_col_iterator(const row_col_iterator &in_it)
    : M(in_it.M), current_pos(in_it.current_pos), internal_col(in_it.internal_col), internal_row(in_it.internal_row)
{
  arma_extra_debug_sigprint();
}

template <typename eT>
inline Mat<eT>::row_col_iterator::row_col_iterator(Mat<eT> &in_M, const uword in_row, const uword in_col)
    : M(&in_M), current_pos(&in_M.at(in_row, in_col)), internal_col(in_col), internal_row(in_row)
{
  arma_extra_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator()
    : M(NULL), current_pos(NULL), internal_col(0), internal_row(0)
{
  arma_extra_debug_sigprint();
  // Technically this iterator is invalid (it does not point to a real element)
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const row_col_iterator &in_it)
    : M(in_it.M), current_pos(in_it.current_pos), internal_col(in_it.col()), internal_row(in_it.row())
{
  arma_extra_debug_sigprint();
}

template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const const_row_col_iterator &in_it)
    : M(in_it.M), current_pos(in_it.current_pos), internal_col(in_it.col()), internal_row(in_it.row())
{
  arma_extra_debug_sigprint();
}
...
template <typename eT>
inline Mat<eT>::const_row_col_iterator::const_row_col_iterator(const Mat<eT> &in_M, const uword in_row, const uword in_col)
    : M(&in_M), current_pos(&in_M.at(in_row, in_col)), internal_col(in_col), internal_row(in_row)
{
  arma_extra_debug_sigprint();
}
...
template <uword fixed_n_rows, uword fixed_n_cols>
template <typename fill_type>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const fill::fill_class<fill_type> &)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols>
template <typename T1>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const Base<eT, T1> &A)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols>
template <typename T1, typename T2>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const Base<pod_type, T1> &A, const Base<pod_type, T2> &B)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const eT *aux_mem)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const char *text)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::string &text)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::initializer_list<eT> &list)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::operator=(const std::initializer_list<eT> &list)
{
  ...
  for (uword iq = N; iq < fixed_n_elem; ++iq)
  {
    this_mem[iq] = eT(0);
  }
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
inline Mat<eT>::fixed<fixed_n_rows, fixed_n_cols>::fixed(const std::initializer_list<std::initializer_list<eT>> &list)
    : Mat<eT>(arma_fixed_indicator(), fixed_n_rows, fixed_n_cols, 0, ((use_extra) ? mem_local_extra : Mat<eT>::mem_local))
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_htrans>
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols>
arma_inline const Op<typename Mat<eT>::template fixed<fixed_n_rows, fixed_n_cols>::Mat_fixed_type, op_strans>

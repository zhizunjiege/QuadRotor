...
internal_approx_equal_abs_diff(const eT &x, const eT &y, const typename get_pod_type<eT>::result tol)
{
  ...
  if (x != y)
  {
    ...
    {
      if (eop_aux::arma_abs((cond_rel<is_not_complex<eT>::value>::gt(x, y)) ? (x - y) : (y - x)) > tol)
      {
        return false;
      }
    }
    ...
    const typename T1::pod_type rel_tol)
{
  ...
  if ((PA.get_n_rows() != PB.get_n_rows()) || (PA.get_n_cols() != PB.get_n_cols()) || (PA.get_n_slices() != PB.get_n_slices()))
  {
    return false;
  }

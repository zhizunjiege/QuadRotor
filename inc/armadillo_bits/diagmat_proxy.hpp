...
public:
  ...
  inline diagmat_proxy_default(const T1 &X)
      : P(X), P_is_vec((resolves_to_vector<T1>::value) || (P.get_n_rows() == 1) || (P.get_n_cols() == 1)), P_is_col(T1::is_col || (P.get_n_cols() == 1)), n_rows(P_is_vec ? P.get_n_elem() : P.get_n_rows()), n_cols(P_is_vec ? P.get_n_elem() : P.get_n_cols())
  {
...
public:
  ...
  inline diagmat_proxy(const Mat<eT> &X)
      : P(X), P_is_vec((X.n_rows == 1) || (X.n_cols == 1)), n_rows(P_is_vec ? X.n_elem : X.n_rows), n_cols(P_is_vec ? X.n_elem : X.n_cols)
  {
    arma_extra_debug_sigprint();
  }
...
public:
  ...
  inline diagmat_proxy_check_default(const T1 &X, const Mat<typename T1::elem_type> &)
      : P(X), P_is_vec((resolves_to_vector<T1>::value) || (P.n_rows == 1) || (P.n_cols == 1)), n_rows(P_is_vec ? P.n_elem : P.n_rows), n_cols(P_is_vec ? P.n_elem : P.n_cols)
  {
    arma_extra_debug_sigprint();
  }
...
public:
  ...
  inline diagmat_proxy_check(const Mat<eT> &X, const Mat<eT> &out)
      : P_local((&X == &out) ? new Mat<eT>(X) : 0), P((&X == &out) ? (*P_local) : X), P_is_vec((P.n_rows == 1) || (P.n_cols == 1)), n_rows(P_is_vec ? P.n_elem : P.n_rows), n_cols(P_is_vec ? P.n_elem : P.n_cols)
  {
    arma_extra_debug_sigprint();
  }
...
public:
  ...
  inline diagmat_proxy_check(const Row<eT> &X, const Mat<eT> &out)
      : P_local((&X == reinterpret_cast<const Row<eT> *>(&out)) ? new Row<eT>(X) : 0), P((&X == reinterpret_cast<const Row<eT> *>(&out)) ? (*P_local) : X), n_rows(X.n_elem), n_cols(X.n_elem)
  {
    arma_extra_debug_sigprint();
  }
...
public:
  ...
  inline diagmat_proxy_check(const Col<eT> &X, const Mat<eT> &out)
      : P_local((&X == reinterpret_cast<const Col<eT> *>(&out)) ? new Col<eT>(X) : 0), P((&X == reinterpret_cast<const Col<eT> *>(&out)) ? (*P_local) : X), n_rows(X.n_elem), n_cols(X.n_elem)
  {
    arma_extra_debug_sigprint();
  }
...
public:
  ...
  inline diagmat_proxy_check(const subview_col<eT> &X, const Mat<eT> &out)
      : P(const_cast<eT *>(X.colptr(0)), X.n_rows, (&(X.m) == &out), false), n_rows(X.n_elem), n_cols(X.n_elem)
  {
    arma_extra_debug_sigprint();
  }

...
arma_warn_unused
    arma_inline const Op<T1, op_max>
    max(
        const T1 &X,
        const uword dim = 0,
...
template <typename T1>
arma_warn_unused
    arma_inline const Op<Op<T1, op_max>, op_max>
...
template <typename T>
arma_warn_unused
    arma_inline const typename arma_scalar_only<T>::result &
    max(const T &x)
{
  return x;
}
...
arma_warn_unused
    ...
    typename enable_if2<
        ...
        const Glue<T1, T2, glue_max>>::result
    max(
        const T1 &X,
        const T2 &Y)
{
...
template <typename T1>
arma_warn_unused
    arma_inline const OpCube<T1, op_max>
    max(
...
template <typename T1, typename T2>
arma_warn_unused
    arma_inline const GlueCube<T1, T2, glue_max>
    max(
        const BaseCube<typename T1::elem_type, T1> &X,
...
template <typename T1>
arma_warn_unused inline const SpOp<SpOp<T1, spop_max>, spop_max>

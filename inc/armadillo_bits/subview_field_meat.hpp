...
subview_field<oT>::subview_field(
    ...
    : f(in_f), aux_row1(in_row1), aux_col1(in_col1), aux_slice1(0), n_rows(in_n_rows), n_cols(in_n_cols), n_slices((in_f.n_slices > 0) ? uword(1) : uword(0)), n_elem(in_n_rows * in_n_cols * n_slices)
{
  arma_extra_debug_sigprint();
}
...
subview_field<oT>::subview_field(
    ...
    : f(in_f), aux_row1(in_row1), aux_col1(in_col1), aux_slice1(in_slice1), n_rows(in_n_rows), n_cols(in_n_cols), n_slices(in_n_slices), n_elem(in_n_rows * in_n_cols * in_n_slices)
{
  arma_extra_debug_sigprint();
}

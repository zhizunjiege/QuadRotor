...
diskio::gen_txt_header(const Mat<eT> &x)
{
  ...
  else if (is_complex_float<eT>::value)
  {
    return std::string("ARMA_MAT_TXT_FC008");
  }
  else if (is_complex_double<eT>::value)
  {
...
diskio::safe_rename(const std::string &old_name, const std::string &new_name)
{
  ...
  if (std::remove(new_name_c_str) != 0)
  {
    return false;
  }
  if (std::rename(old_name.c_str(), new_name_c_str) != 0)
  {
    return false;
  }
...
diskio::convert_naninf(eT &val, const std::string &token)
{
  ...
  if ((token.length() == 3) || (token.length() == 4))
  {
    ...
    else if ((token2 == "nan") || (token2 == "Nan") || (token2 == "NaN") || (token2 == "NAN"))
    {
      val = Datum<eT>::nan;
    ...
    while (line_stream.good() == true)
    {
      ...
      else
      {
        ...
        if (found_x)
        {
          if (loc_x > 0)
          {
            str_real = token.substr(0, loc_x);
          }
          else
          {
            str_real.clear();
          }
          if ((loc_x + 1) < token.size())
          {
            str_imag = token.substr(loc_x, token.size() - loc_x - 1);
          }
          else
          {
            str_imag.clear();
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    if (fid >= 0)
    {
      ...
      if (dataset >= 0)
      {
        ...
        {
          dims[1] = 1;
        } // Vector case; fake second dimension (one column).
...
diskio::load_auto_detect(Mat<eT> &x, const std::string &name, std::string &err_msg)
{
  ...
  if (arma_H5Fis_hdf5(name.c_str()))
  {
    return load_hdf5_binary(x, name, err_msg);
  }
#endif
...
diskio::load_auto_detect(Mat<eT> &x, std::istream &f, std::string &err_msg)
{
  ...
  else if (ARMA_MAT_BIN == header.substr(0, ARMA_MAT_BIN.length()))
  {
    return load_arma_binary(x, f, err_msg);
  }
  else if (P5 == header.substr(0, P5.length()))
  {
...
#if defined(ARMA_USE_HDF5)
  {
    ...
    if (fid >= 0)
    {
      ...
      if (dataset >= 0)
      {
        ...
        {
          dims[2] = 1;
        } // Matrix case; one column, several rows/slices
...
diskio::load_auto_detect(Cube<eT> &x, std::istream &f, std::string &err_msg)
{
  ...
  else if (ARMA_CUB_BIN == header.substr(0, ARMA_CUB_BIN.length()))
  {
    return load_arma_binary(x, f, err_msg);
  }
  else if (P6 == header.substr(0, P6.length()))
  {
...
diskio::load_auto_detect(field<T1> &x, std::istream &f, std::string &err_msg)
{
  ...
  else if (ARMA_FL3_BIN == header.substr(0, ARMA_FL3_BIN.length()))
  {
    return load_arma_binary(x, f, err_msg);
  }
  else if (P6 == header.substr(0, P6.length()))

...
arma_check(const bool state, const T1 &x)
{
  if (state)
  {
    arma_stop_logic_error(arma_str::str_wrapper(x));
  }
}
...
arma_cold
    arma_noinline static std::string
    ...
    if (M_vec_state == 0)
    {
      if (
          (
              ...
              ((Q_n_cols == M_n_rows) && (Q_n_slices == M_n_cols))) == false)
      {
        std::stringstream tmp;
...
namespace junk
{
  ...
  {
  public:
    inline arma_first_extra_debug_message()
    {
      union
      {
        unsigned short a;

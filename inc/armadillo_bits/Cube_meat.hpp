...
template <typename eT>
inline Cube<eT>::Cube()
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
}
...
template <typename eT>
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem_slice(in_n_rows * in_n_cols), n_slices(in_n_slices), n_elem(in_n_rows * in_n_cols * in_n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Cube<eT>::Cube(const SizeCube &s)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem_slice(s.n_rows * s.n_cols), n_slices(s.n_slices), n_elem(s.n_rows * s.n_cols * s.n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
inline Cube<eT>::Cube(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices, const fill::fill_class<fill_type> &)
    : n_rows(in_n_rows), n_cols(in_n_cols), n_elem_slice(in_n_rows * in_n_cols), n_slices(in_n_slices), n_elem(in_n_rows * in_n_cols * in_n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename fill_type>
inline Cube<eT>::Cube(const SizeCube &s, const fill::fill_class<fill_type> &)
    : n_rows(s.n_rows), n_cols(s.n_cols), n_elem_slice(s.n_rows * s.n_cols), n_slices(s.n_slices), n_elem(s.n_rows * s.n_cols * s.n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Cube<eT>::Cube(Cube<eT> &&in_cube)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Cube<eT>::Cube(const Cube<eT> &x)
    : n_rows(x.n_rows), n_cols(x.n_cols), n_elem_slice(x.n_elem_slice), n_slices(x.n_slices), n_elem(x.n_elem), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Cube<eT>::Cube(eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols, const uword aux_n_slices, const bool copy_aux_mem, const bool strict, const bool prealloc_mat)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem_slice(aux_n_rows * aux_n_cols), n_slices(aux_n_slices), n_elem(aux_n_rows * aux_n_cols * aux_n_slices), mem_state(copy_aux_mem ? 0 : (strict ? 2 : 1)), mem(copy_aux_mem ? 0 : aux_mem), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);

  if (prealloc_mat == true)
  {
    arma_debug_warn("Cube::Cube(): parameter 'prealloc_mat' ignored as it's no longer used");
  }
...
//! the array is copied.
template <typename eT>
inline Cube<eT>::Cube(const eT *aux_mem, const uword aux_n_rows, const uword aux_n_cols, const uword aux_n_slices)
    : n_rows(aux_n_rows), n_cols(aux_n_cols), n_elem_slice(aux_n_rows * aux_n_cols), n_slices(aux_n_slices), n_elem(aux_n_rows * aux_n_cols * aux_n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
inline Cube<eT>::Cube(const subview_cube<eT> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem_slice(X.n_elem_slice), n_slices(X.n_slices), n_elem(X.n_elem), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
//! creation of subview_cube (subcube comprised of specified slices)
template <typename eT>
arma_inline const subview_cube<eT>
...
Cube<eT>::subcube(const uword in_row1, const uword in_col1, const uword in_slice1, const SizeCube &s)
{
  ...
  arma_debug_check(
      (in_row1 >= l_n_rows) || (in_col1 >= l_n_cols) || (in_slice1 >= l_n_slices) || ((in_row1 + s_n_rows) > l_n_rows) || ((in_col1 + s_n_cols) > l_n_cols) || ((in_slice1 + s_n_slices) > l_n_slices),
...
template <typename eT>
arma_inline const subview_cube_each1<eT>
...
template <typename eT>
template <typename T1>
inline subview_cube_each2<eT, T1>
...
template <typename eT>
template <typename T1>
inline const subview_cube_each2<eT, T1>
...
inline Cube<eT>::Cube(const GenCube<eT, gen_type> &X)
    : n_rows(X.n_rows), n_cols(X.n_cols), n_elem_slice(X.n_rows * X.n_cols), n_slices(X.n_slices), n_elem(X.n_rows * X.n_cols * X.n_slices), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
inline Cube<eT>::Cube(const OpCube<T1, op_type> &X)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
inline Cube<eT>::Cube(const eOpCube<T1, eop_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()), n_elem_slice(X.get_n_elem_slice()), n_slices(X.get_n_slices()), n_elem(X.get_n_elem()), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename op_type>
inline Cube<eT>::Cube(const mtOpCube<eT, T1, op_type> &X)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
inline Cube<eT>::Cube(const GlueCube<T1, T2, glue_type> &X)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
  this->operator=(X);
...
inline Cube<eT>::Cube(const eGlueCube<T1, T2, eglue_type> &X)
    : n_rows(X.get_n_rows()), n_cols(X.get_n_cols()), n_elem_slice(X.get_n_elem_slice()), n_slices(X.get_n_slices()), n_elem(X.get_n_elem()), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <typename T1, typename T2, typename glue_type>
inline Cube<eT>::Cube(const mtGlueCube<eT, T1, T2, glue_type> &X)
    : n_rows(0), n_cols(0), n_elem_slice(0), n_slices(0), n_elem(0), mem_state(0), mem(), mat_ptrs(0)
{
  arma_extra_debug_sigprint_this(this);
...
Cube<eT>::in_range(const uword in_row, const uword in_col, const uword in_slice, const SizeCube &s) const
{
  ...
  if (
      (in_row >= l_n_rows) || (in_col >= l_n_cols) || (in_slice >= l_n_slices) || ((in_row + s.n_rows) > l_n_rows) || ((in_col + s.n_cols) > l_n_cols) || ((in_slice + s.n_slices) > l_n_slices))
  {
    return false;
  }
  else
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed()
{
  arma_extra_debug_sigprint_this(this);
...
template <typename eT>
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices> &X)
{
  arma_extra_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
template <typename fill_type>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const fill::fill_class<fill_type> &)
{
  arma_extra_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
template <typename T1>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const BaseCube<eT, T1> &A)
{
  arma_extra_debug_sigprint_this(this);
...
template <uword fixed_n_rows, uword fixed_n_cols, uword fixed_n_slices>
template <typename T1, typename T2>
inline Cube<eT>::fixed<fixed_n_rows, fixed_n_cols, fixed_n_slices>::fixed(const BaseCube<pod_type, T1> &A, const BaseCube<pod_type, T2> &B)
{
  arma_extra_debug_sigprint_this(this);

...
Gen<T1, gen_type>::apply(Mat<typename T1::elem_type> &out) const
{
  ...
  else if (is_same_type<gen_type, gen_ones>::yes)
  {
    out.ones();
  }
  else if (is_same_type<gen_type, gen_zeros>::yes)
  {
    out.zeros();
  }
  else if (is_same_type<gen_type, gen_randu>::yes)
  {
    out.randu();
  }
  else if (is_same_type<gen_type, gen_randn>::yes)
  {
    out.randn();
  }
}
...
Gen<T1, gen_type>::apply_inplace_schur(Mat<typename T1::elem_type> &out) const
{
  ...
  if (is_same_type<gen_type, gen_eye>::yes)
  {
    ...
    for (uword iq = 0; iq < N; ++iq)
    {
      for (uword row = 0; row < iq; ++row)
      {
        out.at(row, iq) = eT(0);
      }
      for (uword row = iq + 1; row < n_rows; ++row)
      {
        out.at(row, iq) = eT(0);
      }
    }

    ...
    const typename arma_not_cx<typename T1::elem_type>::result junk2)
{
  ...
  if (Proxy<T1>::use_at == false)
  {
    ...
    for (uword i = 0; i < n_elem; ++i)
    {
      ...
      if (is_same_type<op_type, op_rel_lt_pre>::yes)
      {
        count += (val < tmp) ? uword(1) : uword(0);
      }
      else if (is_same_type<op_type, op_rel_lt_post>::yes)
      {
        count += (tmp < val) ? uword(1) : uword(0);
      }
      ...
      else if (is_same_type<op_type, op_rel_eq>::yes)
      {
        count += (tmp == val) ? uword(1) : uword(0);
      }
    ...
    const typename arma_not_cx<typename T2::elem_type>::result junk3)
{
  ...
  if (use_at == false)
  {
    ...
    for (uword i = 0; i < n_elem; ++i)
    {
      ...
      if (is_same_type<glue_type, glue_rel_lt>::yes)
      {
        count += (tmp1 < tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_gt>::yes)
      {
        count += (tmp1 > tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_lteq>::yes)
      {
        count += (tmp1 <= tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_gteq>::yes)
      {
        count += (tmp1 >= tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_eq>::yes)
      {
        count += (tmp1 == tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_noteq>::yes)
      {
        count += (tmp1 != tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_and>::yes)
      {
        count += (tmp1 && tmp2) ? uword(1) : uword(0);
      }
      else if (is_same_type<glue_type, glue_rel_or>::yes)
      {
        count += (tmp1 || tmp2) ? uword(1) : uword(0);
      }
    }

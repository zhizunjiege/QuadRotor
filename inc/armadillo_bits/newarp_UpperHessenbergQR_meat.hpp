...
namespace newarp
{
  ...
  inline UpperHessenbergQR<eT>::UpperHessenbergQR()
      : n(0), computed(false)
  {
  ...
  template <typename eT>
  inline UpperHessenbergQR<eT>::UpperHessenbergQR(const Mat<eT> &mat_obj)
      : n(mat_obj.n_rows), mat_T(n, n), rot_cos(n - 1), rot_sin(n - 1), computed(false)
  {
    arma_extra_debug_sigprint();
  ...
  template <typename eT>
  inline TridiagQR<eT>::TridiagQR(const Mat<eT> &mat_obj)
      : UpperHessenbergQR<eT>()
  {
    arma_extra_debug_sigprint();

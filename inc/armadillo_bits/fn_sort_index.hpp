...
arma_warn_unused
    arma_inline const mtOp<uword, T1, op_sort_index>
    sort_index(
...
template <typename T1>
arma_deprecated inline const mtOp<uword, T1, op_sort_index>
sort_index(
    const Base<typename T1::elem_type, T1> &X,
...
template <typename T1>
arma_warn_unused
    arma_inline const mtOp<uword, T1, op_stable_sort_index>
    stable_sort_index(
        const Base<typename T1::elem_type, T1> &X)
{
  arma_extra_debug_sigprint();
...
//! NOTE: don't use this form: it will be removed
template <typename T1>
arma_deprecated inline const mtOp<uword, T1, op_stable_sort_index>
stable_sort_index(
    const Base<typename T1::elem_type, T1> &X,
    const uword sort_type)
{

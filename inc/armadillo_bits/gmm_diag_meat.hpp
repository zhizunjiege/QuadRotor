...
namespace gmm_priv
{
  ...
  template <typename eT>
  inline gmm_diag<eT>::gmm_diag(const gmm_diag<eT> &x)
  {
    arma_extra_debug_sigprint_this(this);
  ...
  template <typename eT>
  inline gmm_diag<eT>::gmm_diag(const uword in_n_dims, const uword in_n_gaus)
  {
    arma_extra_debug_sigprint_this(this);
      ...
      const bool print_mode)
  {
    ...
    const bool seed_mode_ok =
        (seed_mode == keep_existing) || (seed_mode == static_subset) || (seed_mode == static_spread) || (seed_mode == random_subset) || (seed_mode == random_spread);
    ...
    else
    {
      ...
      if (print_mode)
      {
        get_stream_err2() << "gmm_diag::learn(): generating initial means\n";
      }
      ...
      else if (dist_mode == maha_dist)
      {
        generate_initial_means<2>(X, seed_mode);
      }
    }
    ...
    if (km_iter > 0)
    {
      ...
      if (dist_mode == eucl_dist)
      {
        status = km_iterate<1>(X, km_iter, print_mode, "gmm_diag::learn(): k-means");
      }
      else if (dist_mode == maha_dist)
      {
        status = km_iterate<2>(X, km_iter, print_mode, "gmm_diag::learn(): k-means");
      }
    ...
    if (seed_mode != keep_existing)
    {
      if (print_mode)
      {
        get_stream_err2() << "gmm_diag::learn(): generating initial covariances\n";
      }

      if (dist_mode == eucl_dist)
      {
        generate_initial_dcovs_and_hefts<1>(X, vfloor);
      }
      else if (dist_mode == maha_dist)
      {
        generate_initial_dcovs_and_hefts<2>(X, vfloor);
      }
    }
      ...
      const bool print_mode)
  {
    ...
    else
    {
      ...
      if (print_mode)
      {
        get_stream_err2() << "kmeans(): generating initial means\n";
      }
...
#pragma omp parallel for schedule(static)
  ...
  gmm_diag<eT>::generate_initial_means(const Mat<eT> &X, const gmm_seed_mode &seed_mode)
  {
    ...
    if ((seed_mode == static_subset) || (seed_mode == random_subset))
    {
      ...
      if (seed_mode == static_subset)
      {
        initial_indices = linspace<uvec>(0, X.n_cols - 1, N_gaus);
      }
      else if (seed_mode == random_subset)
      {
        initial_indices = uvec(sort_index(randu<vec>(X.n_cols))).rows(0, N_gaus - 1);
      }
    ...
    else if ((seed_mode == static_spread) || (seed_mode == random_spread))
    {
      ...
      if (seed_mode == static_spread)
      {
        start_index = X.n_cols / 2;
      }
      else if (seed_mode == random_spread)
      {
        start_index = as_scalar(randi<uvec>(1, distr_param(0, X.n_cols - 1)));
      }
  ...
  gmm_diag<eT>::km_iterate(const Mat<eT> &X, const uword max_iter, const bool verbose, const char *signature)
  {
    ...
    for (uword t = 0; t < n_threads; ++t)
    {
      t_running_means[t].set_size(N_gaus);
    }
...
#pragma omp parallel for schedule(static)
        ...
        for (uword g = 0; g < N_gaus; ++g)
        {
          ...
          for (uword t = 0; t < n_threads; ++t)
          {
            total_count += t_running_means[t][g].count();
          }
          ...
          if (dead == false)
          {
            running_means[g](tmp_mean, last_index);
          }
        }
      }
#else
      ...
      if (n_dead_means > 0)
      {
        if (verbose)
        {
          get_stream_err2() << signature << ": recovering from dead means\n";
        }
        ...
        if (n_dead_means == 1)
        {
          ...
          if ((populous_count <= 2) || (dead_g == populous_g))
          {
            return false;
          }
        ...
        else
        {
          ...
          for (uword live_g = 0; live_g < N_gaus; ++live_g)
          {
            if (running_means(live_g).count() >= 2)
            {
              ...
              {
                if (running_means(dead_g).count() == 0)
                {
                  break;
                }
              }
          ...
          if (n_resurrected_means != n_dead_means)
          {
            if (verbose)
            {
              get_stream_err2() << signature << ": WARNING: did not resurrect all dead means\n";
            }
          }
        }
      }
  ...
  gmm_diag<eT>::em_iterate(const Mat<eT> &X, const uword max_iter, const eT var_floor, const bool verbose)
  {
    ...
    for (uword iter = 1; iter <= max_iter; ++iter)
    {
      ...
      if (is_finite(new_avg_log_p) == false)
      {
        return false;
      }
    ...
    if (any(vectorise(dcovs) <= eT(0)))
    {
      return false;
    }
    if (means.is_finite() == false)
    {
      return false;
    }
    if (dcovs.is_finite() == false)
    {
      return false;
    }
    if (hefts.is_finite() == false)
    {
      return false;
    }
...
#pragma omp parallel for schedule(static)
  ...
  gmm_diag<eT>::em_fix_params(const eT var_floor)
  {
    ...
    for (uword g = 0; g < N_gaus; ++g)
    {
      ...
      for (uword d = 0; d < N_dims; ++d)
      {
        if (dcov_mem[d] < var_floor)
        {
          dcov_mem[d] = var_floor;
        }
      }
    }

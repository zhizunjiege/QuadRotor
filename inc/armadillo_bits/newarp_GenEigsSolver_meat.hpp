...
namespace newarp
{
  ...
  template <typename eT, int SelectionRule, typename OpType>
  inline GenEigsSolver<eT, SelectionRule, OpType>::GenEigsSolver(const OpType &op_, uword nev_, uword ncv_)
      : op(op_), nev(nev_), dim_n(op.n_rows), ncv(ncv_ > dim_n ? dim_n : ncv_), nmatop(0), niter(0), eps(std::numeric_limits<eT>::epsilon()), approx0(std::pow(eps, eT(2.0) / 3))
  {
    arma_extra_debug_sigprint();

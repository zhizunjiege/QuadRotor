...
inline subview_cube<eT>::~subview_cube()
{
...
subview_cube<eT>::subview_cube(
    ...
    : m(in_m), aux_row1(in_row1), aux_col1(in_col1), aux_slice1(in_slice1), n_rows(in_n_rows), n_cols(in_n_cols), n_elem_slice(in_n_rows * in_n_cols), n_slices(in_n_slices), n_elem(n_elem_slice * in_n_slices)
{
  arma_extra_debug_sigprint();
}
...
subview_cube<eT>::operator=(const Base<eT, T1> &in)
{
  ...
  else if ((t_n_rows == x_n_rows) && (t_n_cols == 1) && (t_n_slices == x_n_cols))
  {
  ...
  else if ((t_n_rows == 1) && (t_n_cols == x_n_rows) && (t_n_slices == x_n_cols))
  {
    Cube<eT> &Q = const_cast<Cube<eT> &>(t.m);
...
subview_cube<eT>::is_finite() const
{
  ...
  for (uword slice = 0; slice < local_n_slices; ++slice)
  {
    for (uword col = 0; col < local_n_cols; ++col)
    {
      if (arrayops::is_finite(slice_colptr(slice, col), local_n_rows) == false)
      {
        return false;
      }
    }
...
subview_cube<eT>::has_inf() const
{
  ...
  for (uword slice = 0; slice < local_n_slices; ++slice)
  {
    for (uword col = 0; col < local_n_cols; ++col)
    {
      if (arrayops::has_inf(slice_colptr(slice, col), local_n_rows))
      {
        return true;
      }
    }
...
subview_cube<eT>::has_nan() const
{
  ...
  for (uword slice = 0; slice < local_n_slices; ++slice)
  {
    for (uword col = 0; col < local_n_cols; ++col)
    {
      if (arrayops::has_nan(slice_colptr(slice, col), local_n_rows))
      {
        return true;
      }
    }

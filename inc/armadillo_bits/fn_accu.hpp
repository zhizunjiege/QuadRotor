...
#pragma omp parallel for schedule(static) num_threads(int(n_threads_use))
      ...
      for (uword i = (n_threads_use * chunk_size); i < n_rows; ++i)
      {
        val += P.at(i, 0);
      }
    }
...
#pragma omp parallel for schedule(static) num_threads(int(n_threads_use))
      ...
      for (uword i = (n_threads_use * chunk_size); i < n_cols; ++i)
      {
        val += P.at(0, i);
      }
    }
...
template <typename T>
arma_warn_unused inline const typename arma_scalar_only<T>::result &
accu(const T &x)
{
  return x;
}

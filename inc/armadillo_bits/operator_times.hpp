...
arma_inline
    typename enable_if2<
        ...
        const mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_times>>::result
    operator*(
        const T1 &X,
...
//! scalar * trans(T1)
template <typename T1>
arma_inline const Op<T1, op_htrans2>
...
inline
    typename enable_if2<
        ...
        const mtGlue<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, glue_mixed_times>>::result
    operator*(
        const T1 &X,
        const T2 &Y)
{
...
template <typename T1, typename T2>
inline const SpGlue<T1, T2, spglue_plus2>
operator*(

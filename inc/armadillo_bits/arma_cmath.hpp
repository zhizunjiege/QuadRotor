...
arma_log1p(const float x)
{
...
#else
  {
    ...
    else if ((x < float(0)) && (-x < std::numeric_limits<float>::epsilon()))
    {
      return x;
    }
    else
...
arma_log1p(const double x)
{
...
#else
  {
    ...
    else if ((x < double(0)) && (-x < std::numeric_limits<double>::epsilon()))
    {
      return x;
    }
    else

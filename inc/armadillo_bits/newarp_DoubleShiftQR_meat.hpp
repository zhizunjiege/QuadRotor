...
namespace newarp
{
  ...
  template <typename eT>
  inline DoubleShiftQR<eT>::DoubleShiftQR(uword size)
      : n(size), prec(std::numeric_limits<eT>::epsilon()), eps_rel(prec), eps_abs(prec), computed(false)
  {
    arma_extra_debug_sigprint();
  }

  template <typename eT>
  inline DoubleShiftQR<eT>::DoubleShiftQR(const Mat<eT> &mat_obj, eT s, eT t)
      : n(mat_obj.n_rows), mat_H(n, n), shift_s(s), shift_t(t), ref_u(3, n), ref_nr(n), prec(std::numeric_limits<eT>::epsilon()), eps_rel(prec), eps_abs(prec), computed(false)
  {
    arma_extra_debug_sigprint();
